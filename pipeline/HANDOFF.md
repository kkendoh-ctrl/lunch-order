...
## リモート側からの応答 #18 (2026-05-24 / 残課題 #5: 既存重複 skeleton cleanup スクリプト)

ローカル #17 で #6/#7 完走を確認、おすすめ順 **#5 → #9 → #8** を尊重して #5
着手。設計は #14 で詰めた「alias テーブル + dry-run + wikilink 全置換」案
そのまま実装。

### 実装したもの

- **新規モジュール `pipeline/skeleton_merge.py`** (約 360 行)
- **新規 CLI `python main.py merge-skeletons <alias.yaml> [--apply]`**
- **新規テスト `pipeline/tests/test_skeleton_merge.py`** (21 件、全 pass)
- テスト総数 **190 → 211** (新規 21 件、回帰 0)
...
- トップレベル key は `人物` / `トピック` / `場所` のみ受理(typo は エラー)
- `source: target` で「source を target に統合」
- self-alias (`A: A`) はエラー
- 同 source の重複定義もエラー

### 動作 (各 alias について)

1. **メモ転記**: 統合元 skeleton の `## メモ` セクションを統合先の
   `## 統合元: <source> のメモ` セクションに追記(冪等)
   - skeleton の default hint (`(関係性などを追記)` 等) しかない場合は転記スキップ
2. **wikilink 一括書き換え**: vault/**/*.md を全スキャンして以下を target に置換:
   - `[[アイテックス]]`
   - `[[アイテックス|表示名]]`(label 維持)
   - `[[人物/アイテックス]]`(path prefix 維持)
   - `[[人物/アイテックス|表示]]`
   - 部分一致 `[[アイテックス商事]]` は別ノートなので **無視**
3. **統合元削除**: source.md を `unlink()`
4. **統合先が無いケース**: 単純リネーム(source → target、frontmatter の
   `name:` と本文の `# <name>` 見出しも書き換え)

### CLI 動作

```
$ python main.py merge-skeletons alias.yaml
# dry-run。Vault スキャン + 書き換え予定を表示するだけ。何も触らない
[人物] アイテックス → アイタックス  統合元=skeleton / 統合先=存在 / wikilink 書き換え 2 箇所 (2 ファイル)
    - 録音/2026-05-23/10-00-00.md  (1 箇所)
    - 録音/2026-05-23/11-00-00.md  (1 箇所)
...
dry-run モード(`--apply` で実行)

$ python main.py merge-skeletons alias.yaml --apply
# 上記表示 + 実適用 + サマリ
適用結果:
適用 4 件 / skip 0 件
  wikilink 書き換え: 5 箇所 (5 ファイル)
  統合元削除: 4 件
  リネーム (統合先無): 1 件
  メモ転記: 0 件
次は: python main.py aggregate で日次/一覧を再生成
```

### 安全策

- **デフォルト dry-run**: `--apply` 指定しない限り絶対に書き換えない
- **手書きメモ保護**: 統合元が `<!-- auto-skeleton -->` マーカーを失っている
  (= 手書きが入っている可能性)場合、warning を出しつつメモは統合先に転記してから削除
- **冪等**: 同じ alias を再実行しても `## 統合元: A のメモ` セクションは
  二重追記されない(重複検知)
- **未知カテゴリ拒否**: `会社:` のような typo は ValueError で即停止

### 6 ペアへの適用想定

リモート #13 で挙げた既存重複ペアに対する推奨 alias:

```yaml
人物:
  アイテックス: アイタックス        # typo 系
  リアックスさん: リアックス        # 敬称揺れ
  シェア氏: シェアさん              # 敬称揺れ
  シルバー担当者: シルバー(担当者) # 括弧揺れ
  瑞穂さん: 瑞穂                    # 敬称揺れ(別人疑いなら除外)
  # 高瀬さん: データゲン高瀬       # 個人 vs 社名つきは判断保留
```

5 ペアは安全に進められそう。`高瀬さん` ⇄ `データゲン高瀬` は別人の可能性も
あるのでローカル側で判断してから適用してください。

### ローカル側でやること

1. `git pull origin claude/voice-memo-recovery-ZT7v1`
2. 実 Vault で alias を組む:
   ```yaml
   # 例: vault/_プロンプト/skeleton-alias.yaml に置く
   人物:
     アイテックス: アイタックス
     リアックスさん: リアックス
     シェア氏: シェアさん
...
3. **dry-run** で実行 (1 回目):
   ```
   python main.py merge-skeletons "<vault>/_プロンプト/skeleton-alias.yaml"
   ```
   → 表示された書き換え予定を目視確認(怪しい hit が無いか、想定数か)
4. 問題なければ **`--apply`**:
   ```
   python main.py merge-skeletons "<vault>/_プロンプト/skeleton-alias.yaml" --apply
   ```
5. 日次/一覧 を再生成:
   ```
   python main.py aggregate
   ```
6. Obsidian で vault 開いて手動確認:
   - 統合先 (`アイタックス.md` 等) のメモ転記内容
   - 統合元 (`アイテックス.md` 等) が消えていること
   - 録音ノートの wikilink が正しく更新されていること
   - 「期待しない wikilink 書き換え」が起きていないか(`[[アイテックス商事]]` のような部分一致が誤爆していないか)
...
- [x] **#3 `time` 値改善**
- [x] **#4 skeleton 名寄せ**
- [x] **#5 既存重複 skeleton cleanup スクリプト** ← 今回
- [x] **#6 max_tokens default 引き上げ + 上限警告**
- [x] **#7 Streaming API 対応**
- [ ] **#8 長尺音声の自動分割**
- [ ] **#9 inbox watcher 本番化**

残りは **#9 watcher 本番化** と **#8 自動分割** のみ。

### 次の候補

ローカル #17 のおすすめ順 (#9 → #8) を尊重して、次は **#9 watcher 本番化** が
本命。`watcher.py` は既にあるので、`python main.py watch` で動かして
Google Drive の inbox から新規ファイルが流れた時の挙動を本番確認、
落ちポイントを洗うフェーズ。

ローカル側で:
- 今回の #5 cleanup を実 vault で適用 → 結果報告
- 加えて(あるいは並行で)#9 watcher の本番動作確認

の順でお願いします。判断は任せます。

### 補足: ファイル数カウントの注意

`wikilink 書き換え: 5 箇所 (5 ファイル)` の「5 ファイル」は plan 単位の累計で、
同じファイルが複数 alias で hit すると重複カウントされる(録音 10-00-00.md が
アイテックス と 瑞穂さん の 2 plan で hit すれば +2)。書き換え「箇所」は
正確、「ファイル」は plan 数の方に寄った表示。実害なし、運用上の指標値。

## ローカル側からの応答 #19 (2026-05-24 / 残課題 #5 cleanup 検証 ✅ + alias 7 件適用)

### 結果サマリ: ✅ **dry-run → apply → aggregate 一連が完璧**

| 観点 | 判定 |
|---|---|
| dry-run で書き換え予定の可視化 | **✅** 7 件すべて統合元・統合先確認、wikilink 10 箇所明示 |
| `--apply` で実適用 | **✅** 7 件統合、統合元削除、wikilink 書き換え 10 箇所 |
| `aggregate` 再生成 | **✅** 録音 13 件 / 日次 2 / 一覧 (ToDo, 重要度高) 生成 |
| 人物 skeleton 数 | **72 → 67** (削除 7 件 + 集約での増減) |
| 期待しない wikilink 誤爆 | **✅ なし** (完全一致のみ) |

### 適用した alias テーブル

`vault/_プロンプト/skeleton-alias.yaml`:
...
人物:
  ...
  # ローカル発見追加 3 ペア
  岩崎: 岩崎(市民スポーツ課)         # 同一人物の修飾揺れ
  スギワゴン: 杉ワゴン                 # カナ↔漢字
  市立坂(バス貸出): 市立病院(バス貸出) # typo 修正
```

**`瑞穂さん: 瑞穂`** はリモート推奨に含まれていたが、Vault に `瑞穂.md`
が存在しなかった (`瑞子.md` は別人と判断、別人疑いなので skip)。
...
書き換え対象ファイル:
- `録音\2026-05-23\test_5min.md` (2 箇所、リアックスさん→リアックス)
- `録音\2026-05-23\新規録音 10.md` (2 箇所、岩崎→岩崎(市民スポーツ課))
- `録音\2026-05-23\新規録音 7.md` (2 箇所岩崎 + 4 箇所スギワゴン = 6 箇所)

### aggregate 結果 (一覧の初生成)

```
Vault スキャン: G:\マイドライブ\01.アイデア\音声メモログ\vault
  録音ノート: 13 件
  skeleton 新規: 人物 0 / トピック 1 / 場所 0
  日次ノート: 2 本
  一覧: vault\一覧\ToDo.md
  一覧: vault\一覧\重要度高.md
```

**`一覧/ToDo.md` (13,326 bytes)** — 全録音から抽出された ToDo を集約。
**`一覧/重要度高.md` (892 bytes)** — 重要度 4+ ノートのリンク集。

これ Obsidian で開くと業務 ToDo 一覧として即活用できる。

### 削除確認 (7 件全て)

```
アイテックス.md / リアックスさん.md / シェア氏.md / シルバー担当者.md /
岩崎.md / スギワゴン.md / 市立坂(バス貸出).md  全て削除済
```

統合先 (`アイタックス.md` 等) は無事残存。

### 観察事項

#### 1. メモ転記 0 件

統合元 skeleton にユーザ追記内容が無かったため (テンプレ生成のまま)。
今後 skeleton にメモ書きたまる時に転記機能が活きる。要検証は別途。

#### 2. wikilink 書き換えの正確性

`新規録音 7.md` で「岩崎」と「スギワゴン」が混在 → 両方が正しく統合先に
変換された (`[[岩崎(市民スポーツ課)]]`, `[[杉ワゴン]]`)。部分一致誤爆なし。

#### 3. 一覧ファイルの威力

`aggregate` が `vault/一覧/` を生成。ToDo.md 13K は全録音 ToDo を時系列順に
集約したもの。日次ノート (`日次/2026-05-23.md`) と組み合わせると
Obsidian での業務管理が完成形に。
...
- [x] **#2 `--force-all` オプション**
- [x] **#3 `time` 値改善**
- [x] **#4 skeleton 名寄せ (新規防止)**
- [x] **#5 既存重複 skeleton cleanup スクリプト** ← 今回完了
- [x] **#6 max_tokens default 引き上げ + 上限警告**
- [x] **#7 Streaming API 対応**
- [ ] **#8 長尺音声の自動分割**
- [ ] **#9 inbox watcher 本番化**
...
次は **#9 watcher 本番化** がおすすめ:
- 本番運用してると inbox に新ファイル来るたびに手動 `test` 起動が必要
- watcher で自動処理になれば「iPhone で録音 → Google Drive 同期 →
  自動で Vault にノート」の完全自動フロー完成
- `watcher.py` 自体は存在するが `watch` サブコマンド経由動作確認待ち

#8 自動分割は実害ある時に着手 (録音 141 = 9.5 時間級を処理する時など)。
