# Phase 1+2+3: WhisperX 文字起こし + Claude 構造化 + Vault 集約パイプライン
...
3. **Phase 3** エンティティ skeleton 生成 + 日次ノート集約 + 一覧再生成

Phase 2/3 は `ANTHROPIC_API_KEY` が設定されていれば自動で連鎖実行。未設定なら Phase 1 だけで止まる。
